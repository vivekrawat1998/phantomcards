...
app.post('/api/submit-form', (req, res) => {
  ...
  const sql = 'INSERT INTO formdata_with_image (name, phonenumber, address, city, pincode, state, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW())';

  pool.query(sql, [name, phonenumber, address, city, pincode, state], (err, results) => {
    if (err) {
    ...
    res.status(201).json({ message: 'Form submitted successfully', id: results.insertId });
  });
});
...
app.post('/api/upload-image/:formId', upload.single('image'), (req, res) => {
  ...
  const checkFormExistenceSQL = 'SELECT id FROM formdata_with_image WHERE id = ?';
  pool.query(checkFormExistenceSQL, [formId], (err, results) => {
    ...
    const updateImageSQL = 'UPDATE formdata_with_image SET image_data = ? WHERE id = ?';
    pool.query(updateImageSQL, [imageBuffer, formId], (err, results) => {
      ...
      if (results.changedRows === 0) {
        ...
        return res.status(404).json({ error: 'Image upload failed, form ID not found or image unchanged' });
      }

      console.log("Image uploaded successfully:", results);
      res.status(200).json({ message: 'Image uploaded successfully', id: formId });
    });
  });
});
